...
description: '从有状态工具、子 Agent 生命周期和权限边界出发，讨论 Execution Harness 应该如何管理模型行为的后果。'
...
同一轮生成的两个 Tool Call 能否并行？已经启动的子 Agent 怎样取消？参数合法的文件读取为什么仍然可能越权？
...
系统不应该消灭模型的自主性。相反，我们需要识别哪些地方适合让模型探索，哪些行动一旦准备改变外部状态，就必须交回确定性系统。

本文把模型与外部环境之间的这层执行系统称为 **Execution Harness**。我所说的 Harness 只包括 Agent Loop、Tool 调度、状态、权限、生命周期和执行记录，不泛指所有“模型之外的工程”。这个词在工程社区中的边界并不统一；例如 OpenAI 对 [Codex Harness](https://openai.com/index/unlocking-the-codex-harness/) 的描述还包括 Thread 持久化、配置认证和客户端协议，而 [SWE-agent](https://arxiv.org/abs/2405.15793) 从学术视角使用的是 Agent-Computer Interface。

这里真正重要的不是名词，而是它承担的职责：

> **Model proposes; the execution harness commits.**
>
> 模型提出候选行动，Execution Harness 决定行动能否、何时以及以什么方式真正发生。
...
它们共同指向一条更具体的边界：模型可以自由产生候选行动，但行动准备产生真实后果时，需要经过一次确定性的提交判断。
...
> **按行为后果分配自主性，并在 Commit Boundary 执行约束。**

模型适合处理开放问题：理解不完整的意图、生成候选计划、从复杂上下文中寻找路径。Execution Harness 适合处理不应该靠运气的问题：权限、状态一致性、资源回收和执行证据。
...
这不是一套已经完成的答案。并行工具如何声明副作用、子 Agent 如何实现真正的结构化并发、权限策略怎样在不同执行层保持一致，都还有很多具体问题值得展开。

后续我会分别讨论其中的技术细节。但在进入实现之前，我想先保留两条原则：**模型负责探索，Execution Harness 负责后果；自主性在 Commit Boundary 之前展开，在它之后服从系统不变量。**
